...
int main(){
    ...
    Model_List.push_back(new Object(glm::vec3(0, -1, -4), shadermodel_list[2]));
    Model_List[1]->scalemat = glm::scale(Model_List[1]->scalemat, glm::vec3(0.15f));

    for (int i = 0; i < 5; i++) {
        Model_List.push_back(new Object(glm::vec3(i - 5, 0, 0), shadermodel_list[0]));
    }
        ...
        if (glfwGetTime() - picking_last_time > 0.25) {
            ...
            if (isModelSelected == true) {
                ...
                for (int i = 0; i < Model_List.size(); i++) {
                    if (Model_List[i] == last_obj) {
                        for (int j = i; j + 1 < Model_List.size(); j++) {
                            Model_List[j] = Model_List[j + 1];
                        }
                        Model_List.pop_back();
                        break;
                    }
            ...
            if (glfwGetKey(window, GLFW_KEY_0) == GLFW_PRESS) {
                ...
                if (currentTime - last_time_0 > 2) {
                    if (isModelSelected == false) {
                        ...
                        Model_List.push_back(new Object(pos, shadermodel_list[0]));
                        cout << "num: " << Model_List.size() << endl;
                    }
