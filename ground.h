...
std::vector<std::vector<uchar> > decode(const std::string path)   //path为图片路径
{
    cv::Mat img = cv::imread(path.c_str());                // 将图片传入Mat容器中
    //       显示原图片
    ...
    std::vector<std::vector<uchar> > array(h, std::vector<uchar>(w));      //初始化二维vector
    for (int i = 0; i < h; i++)
    {
...
std::vector<std::vector<glm::vec3> > decode2(const std::string path,float scale = 1.0f)   //path为图片路径
{
    ...
    int w = img.cols;     //可能为3通道，宽度要乘图片的通道数
    int h = img.rows;
    int cen_w = w / 2,cen_h = h/2;

    std::vector<std::vector<glm::vec3> > array(h, std::vector<glm::vec3>(w));      //初始化二维vector
    for (int i = 0; i < h; i++)
    {
        uchar* inData = img.ptr<uchar>(i);            //ptr为指向图片的行指针，参数i为行数
        for (int j = 0; j < w; j++)
